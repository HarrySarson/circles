...
module.exports = exports = inherit.parent(Item).child(function(coor,theta,selected){
    ...
    var mouse = {x: Math.cos(this.theta), y: Math.sin(this.theta)};
...
exports.prototype.clickbox = function(x,y){
    ...
    var tipdelta   = { x: this._arrowLength * Math.cos(this.theta), y: this._arrowLength * Math.sin(this.theta) };
    var mousedelta = { x: x - this.end.x, y: y - this.end.y };
    ...
    return (distfrompoint <= arrowside || (distfromline <= arrowwidth && frac > 0 && frac < 1)) ? 
            { d: distfrompoint, type: arrowType, id:0 } : 
            false;
};
...
exports.prototype.draw = function(ui,colors){
    ...
    if(this._selected.is_type(arrowType)){
        ...
        if(this._selected.is_type(arrowType)){
            var a = 20/*this._arrowLength*pointlength*/, b = a/(3+0.25);
            // plot angle
            ui.context.save();
