...
animate(function(controller){
    ...
    var initial = { x: 0, y: 0,
            width: $main.outerWidth(), height: $main.outerHeight() };
    ...
    var b = 0.1;
    var adjust = {};
    adjust.x = smoother({a:1, b: b, c: b*b/4});
    adjust.y = smoother({a:1, b: b, c: b*b/4});
...
animate(function(controller){
    ...
    force_replot = function() { firstplot = true; }
    
    
    var items = {
        bin:    new ItemCtor.Bin(selected, {x:30, y: -60}, 35, 60, {x:0, y:1})
    };
    
    
    items.points = new ItemCtor.Points(selected,items.bin,function(x){ return 1/(cosh(0.01*x*x  )); }),
    ...
    var getcoor = function(e){
        var coor = { x:e.pageX - offset.left, y: e.pageY - offset.top };
