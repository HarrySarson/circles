...
module.exports = exports = inherit.parent(Item).child(function(points, arrow, selected, options) {
...
var get_circle = function(coor0, coor1, theta, centres) {
  var dx = coor1.x - coor0.x,
    dy = coor1.y - coor0.y,
  ...
  var previousClockwise = centres.back() ? centres.back().clockwise : false;
...
exports.prototype.recalc = function() {
  var theta = this.arrow.theta,
    lastcoor = this.points.coor(0);

  this.centres.reset();

  for (var i = 1, l = this.points.length; i < l + complete; ++i) {
    var newcoor = this.points.coor(i % l)
...
exports.prototype.draw = function(ui, colors) {
  ...
  for (var i = 1, l = this.points.length, centreCount = 0; i < l + complete;
    ++i) {
    var newcoor = self.points.coor(i % self.points.length);
    if (!newcoor.binned) {
      var x0 = lastcoor.x,
        y0 = lastcoor.y,
        x1 = newcoor.x,
